...
print("Starting unit tests for SensorData object")
epsilon = 1e-5
...
reset_test_folder()
sensor_data = SensorData(f"{test_folder_path}/TEST_calibration_data.csv", f"{test_folder_path}/TEST_sensor_data.csv", f"{test_folder_path}/TEST_ph_data.csv")
test_data = rand(3, N_ELECTRODES)
test_data.sort(axis = 0)
ph_1 = 2
ph_2 = 12
sensor_data.add_calibration(ph_1, test_data[0])
sensor_data.add_calibration(ph_2, test_data[2])
guid = sensor_data.add_measurement(test_data[1])
for electrode_id in range(N_ELECTRODES):
    ...
    result = sensor_data.ph_data[sensor_data.ph_data["guid"] == guid][f"ph_electrode_{electrode_id}"].iloc[0]
...
reset_test_folder()
sensor_data = sensor_data(f"{test_folder_path}/TEST_calibration_data.csv", f"{test_folder_path}/TEST_sensor_data.csv", f"{test_folder_path}/TEST_ph_data.csv")
...
sensor_data.add_calibration(ph_0, test_data[0])
sensor_data.add_calibration(ph_1, test_data[1])
sensor_data.add_calibration(ph_2, test_data[2])
# sensor_data.add_calibration(ph_3, test_data[3])
sensor_data.add_calibration(ph_4, test_data[4])
sensor_data.add_calibration(ph_5, test_data[5])
sensor_data.add_calibration(ph_6, test_data[6])

guid = sensor_data.add_measurement(test_data[3])
for electrode_id in range(N_ELECTRODES):
...
for i in range(5):
    sensor_data.add_calibration(ph_1 + uniform(-0.4, 0.4), rand(N_ELECTRODES) * 5)
    sensor_data.add_calibration(ph_2 + uniform(-0.4, 0.4), rand(N_ELECTRODES) * 5)
...
def assign_calibrations(ph_value, test_data_row):
    ...
    for i in range(int(N_ELECTRODES / n_electrodes_per_calibration)):
        ...
        # print(data_to_add)
        sensor_data.add_calibration(ph_value, data_to_add)
